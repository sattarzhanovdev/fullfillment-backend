...
async function main() {
    ...
    const clientDefs = [
        { name: 'ООО "Ромашка"', type: 'OOO', debtLimit: 15000, bufferPercent: 15, priceFormula: { first: 20, next: 7 } },
        { name: 'ИП Соколова', type: 'IP', debtLimit: 8000, bufferPercent: null, flatPrice: 0 },
        { name: 'ООО "Технопарк"', type: 'OOO', debtLimit: 20000, bufferPercent: null },
        { name: 'ИП Волков', type: 'IP', debtLimit: 2000, bufferPercent: null },
    ];
    ...
    productsByClient.set(existingClient.id, (productsByClient.get(existingClient.id) ?? []).concat(existingProducts.map((p) => ({ id: p.id, barcode: p.barcode }))));
    async function shipProductHistorical(productId, qty, at, reference) {
        ...
        for (let i = 0; i < ordersToday; i += 1) {
            ...
            const order = await prisma.marketplaceOrder.create({
                data: {
                    ...
                    marketplace: client_1.Marketplace.WILDBERRIES,
                    clientId: client.id,
                    status: 'COMPLETED',
                    priority: 'NORMAL',
    ...
    for (const def of supplyStatuses) {
        ...
        await prisma.supplyStatusHistory.create({ data: { supplyId: supply.id, status: def.status, createdAt: supplyDate } });
    }
